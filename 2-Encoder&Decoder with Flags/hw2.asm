...
uitoa:
   	...
   	# $a2, uitoa_outputSize
    	...
    	move $t2, $a0	# Temp store for $a0
    	move $t8, $a1	# Temp store for $a1
...
runLengthEncode:
	# $a0, runLengthEncode_input
    	...
    	runLengthEncode.buildString:
    		li $t9, 3	
    		ble $t1, $t9, buildSmall	# Branch if RUN COUNTER <=3
    		...
    		li $a1, 0
    		add $a1, $a1, $t1	# $a1, encodeRun_runLength int
    	 ...
    	 buildSmall:
    	 	sb $t0, 0($a1)
    	 	addi $a1, $a1, 1	# advance output memory
    	 	addi $t1, $t1, -1
    	 	beqz $t1, section1
    	 	j buildSmall
